---
title: Variation in dietary fiber content in 282 common bean genotypes from the middle
  american gene pool
author: "skammlade"
date: "17 June 2016"
output:
...
`r nrow(fiber)/2` entries from the BeanCAP study were grown in Fort Collins, CO in 2015. All entries originated from the Middle American gene pool and were further subdivided into three races and twelve market classes (Table 1). Beans were analyzed for various fiber components to determine the effect of marketclass and race (Table 2).

|Durango                  | |Jalisco                 |  |Mesoamerican                  |  |             
| ------------ | ----------- | ------------ |------------ | --------------- | ----------- |
| Market Class | No. Entries | Market Class | No. Entries | Market Class | No. Entries | 
| ------------ | ----------- | ------------ |------------ | ------------ | ----------- |
| GN | `r sum(fiber$mktclass=="GN", na.rm=TRUE)/2` |  black mottle | `r sum(fiber$mktclass=="black mottle", na.rm=TRUE)/2` | black     |   `r sum(fiber$mktclass=="black", na.rm=TRUE)/2`                                          
| pinto | `r sum(fiber$mktclass=="pinto", na.rm=TRUE)/2` | flor de mayo | `r sum(fiber$mktclass=="flor de mayo", na.rm=TRUE)/2` | carioca     |`r sum(fiber$mktclass=="carioca", na.rm=TRUE)/2`                                             
|       |                                             | pink | `r sum(fiber$mktclass=="pink", na.rm=TRUE)/2`                 | navy      | `r sum(fiber$mktclass=="navy", na.rm=TRUE)/2`                                            
|       |                                             | red mottle | `r sum(fiber$mktclass=="red mottle", na.rm=TRUE)/2`     | small white     |  `r sum(fiber$mktclass=="small white", na.rm=TRUE)/2`                                           
|       |                                             | small red | `r sum(fiber$mktclass=="small red", na.rm=TRUE)/2`       |  tan     |  `r sum(fiber$mktclass=="tan", na.rm=TRUE)/2` 
| **Totals** | **`r sum(fiber$race=="durango", na.rm=TRUE)/2`** |           | **`r sum(fiber$race=="jalisco", na.rm=TRUE)/2`** |                      | **`r sum(fiber$race=="mesoamerican", na.rm=TRUE)/2`**
...
Table: Fiber components measured in the study. Verbascose, raffinose, and stachyose are grouped together broadly as total oligosaccharides. These carbohydrates are not digested in the small intestine and are instead passed into the large intestine to be fermented by microflora with the production of gas.
...
One-way analysis of variance was conducted to determine the effect of 1)market class and 2) race on fiber components of the `r nrow(fiber)/2` entries using the aov function.
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
source('anova.mktclass.R')
library(xtable, knitr)
kable(xtable(OneWayFit.IDF), caption="One-way ANOVA table testing differences in insoluble DF between market classes.", digits=4) 
kable(xtable(OneWayFit.SDF), caption="One-way ANOVA table testing differences in soluble DF between market classes.", digits=4)
kable(xtable(OneWayFit.Verb), caption="One-way ANOVA table testing differences in verbascose between market classes.", digits=4)
kable(xtable(OneWayFit.Raff), caption="One-way ANOVA table testing differences in raffinose DF between market classes.", digits=4)
kable(xtable(OneWayFit.Stach), caption="One-way ANOVA table testing differences in stachyose DF between market classes.", digits=4)
kable(xtable(OneWayFit.TOligos), caption="One-way ANOVA table testing differences in total oligosaccharides DF between market classes.", digits=4)
kable(xtable(OneWayFit.TDF), caption="One-way ANOVA table testing differences in total DF between market classes.", digits=4)
```

#### *2) Race*

Differences between races were significant (p<0.05) across all fiber components except total oligosaccharides.

```{r echo=FALSE, message=FALSE, warning=FALSE}
source('anova.race.R')
library(xtable, knitr)
kable(xtable(OneWayFit.IDF), caption="One-way ANOVA table testing differences in insoluble DF between races.", digits=4)
kable(xtable(OneWayFit.SDF), caption="One-way ANOVA table testing differences in soluble DF between races.", digits=4)
kable(xtable(OneWayFit.Verb), caption="One-way ANOVA table testing differences in verbascose between races.", digits=4)
kable(xtable(OneWayFit.Raff), caption="One-way ANOVA table testing differences in raffinose between races.", digits=4)
kable(xtable(OneWayFit.Stach), caption="One-way ANOVA table testing differences in stachyose between races.", digits=4)
kable(xtable(OneWayFit.TOligos), caption="One-way ANOVA table testing differences in total oligosaccharides between races.", digits=4)
kable(xtable(OneWayFit.TDF), caption="One-way ANOVA table testing differences in total DF between races.", digits=4)
```
