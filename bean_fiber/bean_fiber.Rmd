...
Insoluble fiber, soluble fiber, raffanose, stachyose, and verbascose data were concluded to meet the assumptions of normality of residuals and equality of variance after plotting qqplots, and residuals versus predicted values. Diagnostic plots are shown below.
...
```{r fig.width = 8, fig.height=9, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Residual versus fitted and q-q plots of verbascose, raffanose, and stachyose. The raffanose plots look to reveal non-homogeneity of variance, but both the log and square root transformations did not improve it enough to warrant transforming the data."}
library(cowplot)
...
kable(xtable(OneWayFit.Raff), caption="One-way ANOVA table testing differences in raffanose DF between market classes.", digits=4)
...
kable(xtable(OneWayFit.Raff), caption="One-way ANOVA table testing differences in raffanose between races.", digits=4)
...
Tukey adjusted pairwise comparisons were made between each market class and each race for each fiber component using the TukeyHSD funcion.
